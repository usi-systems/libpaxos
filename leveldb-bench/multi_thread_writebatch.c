...
#define TEST_DB "/tmp/test_leveldb"
...
int main(int argc, char* argv[])
{
  ...
  char *stat = leveldb_property_value(ctx->db, "leveldb.stats");
  printf("%s\n", stat);
  free(stat);
  // char *sstable = leveldb_property_value(ctx->db, "leveldb.sstables");
  // printf("%s\n", sstable);
  // free(sstable);
  leveldb_close(ctx->db);
  destroy_db(ctx, TEST_DB);
...
void *consumer_task(void *arg)
{
  ...
  for (int i=0; i < NUM_KEYS; i++)
  {
    ...
    leveldb_writebatch_put(batch, key, strlen(key), value, (strlen(value) + 1));
    if (i%BATCH_SIZE == 0) {
      leveldb_write(ctx->db, ctx->woptions, batch, &err);
      if (err != NULL) {
        fprintf(stderr, "Write fail.\n");
        break;
      }
