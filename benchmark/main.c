...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i < NUM_THREADS; i++)
    {
        ...
        pthread_mutex_init(&parms[i].mutex, NULL);
        ret = pthread_create(&producers[i], NULL, produce_accepted, (void*)&parms[i]);
        if (ret)
        {
...
void *print_stat(void *arg)
{
    ...
    while (!force_quit)
    {
        sleep(1);
        pthread_mutex_lock(&parm->mutex);
        printf("Thread %d Throughput: %d pps\n", parm->producer_id, parm->delivered_total);
        parm->delivered_total = 0;
        pthread_mutex_unlock(&parm->mutex);
    }
    return NULL;
}
